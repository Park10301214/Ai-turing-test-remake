...
const EXPERIMENT_SIZE = 16;

let participant = {name:"",number:""};
let trials = [];
let answers = [];
let index = 0;
...
function createTrials(){
  ...
  QUESTION_CATEGORIES.forEach(category=>{
    const pool=shuffle(QUESTION_DATA.filter(q=>q.category===category));
    ["ChatGPT","Gemini","Claude","Human"].forEach((source,i)=>{
...
function startExperiment(e){
  ...
  const number=Number($("participant-number").value);
  if(!name){$("start-error").textContent="참가자 이름 또는 별명을 입력해 주세요.";return;}
  if(!Number.isInteger(number)||number<1||number>10){$("start-error").textContent="참가자 번호는 1부터 10 사이의 정수로 입력해 주세요.";return;}
...
function saveAnswer(){
  ...
  answers.push({
    ...
    reason:$("judgment-reason").value.trim(),
...
function finishExperiment(){
  ...
  $("result-participant").textContent=`${participant.name} 참가자 · ${participant.number}번`;
...
function saveSession(){
  const all=getStored();
  all.push({
    sessionId:`${participant.number}_${Date.now()}`,
    participant,
...
$("download-csv").addEventListener("click",()=>downloadCsv(`participant_${participant.number}_${participant.name}.csv`,answers));
$("restart-button").addEventListener("click",()=>{showScreen("start");$("participant-name").value="";$("participant-number").value=""});
